...
UserDefinedMap fake_fields_map({{Field_fake::points, "points"}});

UserDefinedMap points_fields_map({{Field_points::x, "x"},
                                  {Field_points::y, "y"},
                                  {Field_points::z, "z"}});

fake_t::fake_t(kaitai::kstream *p__io, kaitai::kstruct *p__parent,
               fake_t *p__root)
...
fake_t::~fake_t() { _clean_up(); }

void fake_t::_clean_up() {
  if (m_points) {
    for (std::vector<point_t *>::iterator it = m_points->begin();
         it != m_points->end(); ++it) {
      delete *it;
    }
    delete m_points;
...
fake_t::point_t::point_t(kaitai::kstream *p__io, fake_t *p__parent,
                         fake_t *p__root)
    : kaitai::kstruct(p__io),
      points_builder(
          p__parent->fake_builder.content<Field_fake::points>().content()) {
  m__parent = p__parent;
  m__root = p__root;
...
fake_t::point_t::~point_t() { _clean_up(); }
...
int64_t num_buffers(void *builder) {
  return awkward::num_buffers_helper(
      reinterpret_cast<FakeBuilderType *>(builder));
}

const char *buffer_name(void *builder, int64_t index) {
  return awkward::buffer_name_helper(
             reinterpret_cast<FakeBuilderType *>(builder))[index]
      .c_str();
}

int64_t buffer_size(void *builder, int64_t index) {
  return awkward::buffer_size_helper(
      reinterpret_cast<FakeBuilderType *>(builder))[index];
}

void copy_into(const char *name, void *from_builder, void *to_buffer,
               int64_t index) {
